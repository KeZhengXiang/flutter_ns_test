...
class PathUtil {
  ...
  void test() async {
    ...
    // clearCache();
    // listDir(await applicationDocumentsDirectory);
  ...
  Future<Null> deleteDir(FileSystemEntity file) async {
    ...
    if (Platform.isIOS) {
      if (file is File) {
        await file.delete();
        logDebug("IOS delete file: ${file.path}, d = ${file is Directory}, f = ${file is File}");
      } else {
        logDebug("IOS 保留 folder: ${file.path}, d = ${file is Directory}, f = ${file is File}");
      }
    } else {
      await file.delete();
      logDebug("Android delete : ${file.path}, d = ${file is Directory}}");
    }
  }
