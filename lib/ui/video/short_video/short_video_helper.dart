...
class ShortVideoHelper {
  ...
  //
  void changeIdx(int curIdx, List<ShortVideoInfo> list, {bool isInit = false}) {
    int startIdx = curIdx;
    // int startIdx = 0;
    if (curIdx > maxCache) {
      startIdx = curIdx - maxCache;
    }
    int endIndex = 0;
    if (curIdx <= (list.length - 1) - maxCache) {
      endIndex = curIdx + maxCache;
    }

    //当前缓存的集合
    List<String> cacheKeys = _videoCtrHashMap.keys.toList();
    //开始缓存【curIdx，endIndex】视频
    for (int i = startIdx; i <= endIndex; ++i) {
      if (!(isInit == false && curIdx == i)) {
        //排除当前播放的，初始化的时候除外
    ...
    logDebug("---9981231--- 当前缓存 【$startIdx, $endIndex】");

    //去掉放弃缓存的
    cacheKeys.forEach((key) {
      final ctr = _videoCtrHashMap[key]!;
      ctr.stopPreCache(_videoSourceHashMap[key]!);
      ctr.dispose();
  ...
  void clear() {
    ...
    _videoCtrHashMap.clear();
    _videoSourceHashMap.clear();
  }

  BetterPlayerController getController(int index, ShortVideoInfo model) {
    final String key = model.mediaUrl!;
    if (_videoCtrHashMap.containsKey(key)) {
      logDebug("---9981231--- index = $index,  获取ctr");
